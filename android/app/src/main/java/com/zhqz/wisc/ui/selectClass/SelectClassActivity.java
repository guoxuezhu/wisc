...
public class SelectClassActivity extends BaseActivity implements SelectClassMVPView {


    @Inject
    SelectClassPresenter mSelectClassPresenter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSelectClassPresenter.attachView(this);
        EventBus.getDefault().register(this);
        ELog.d("====绑定教室==tjj2=" + WiscApplication.prefs.getIsSelectKaoQing()+"====="+WiscApplication.prefs.getisBind());
//        if (WiscApplication.prefs.getIsSelectKaoQing() ) {
//            WiscApplication.prefs.setSceneId(2);
//            ELog.d("====绑定教室==tjj2222=" + WiscApplication.prefs.getIsSelectKaoQing());
//
//            startActivity(new Intent(SelectClassActivity.this, MainActivity.class));
//            finish();
//            return;
//        }
        if (WiscApplication.prefs.getisBind()){
            if (WiscApplication.prefs.getIsSelectKaoQing() ) {
                WiscApplication.prefs.setSceneId(2);
                ELog.d("====绑定教室==tjj2222=" + WiscApplication.prefs.getIsSelectKaoQing());
                startActivity(new Intent(SelectClassActivity.this, MainActivity.class));
            ...
            mSelectClassPresenter.classRooms();
        }
                ...
                switch (promptDialog.mIndex1) {
                    case 1://确认
                        if(WiscApplication.prefs.getclassRoomId()==-1){
                            Toast.makeText(SelectClassActivity.this, getResources().getString(R.string.no_select_room), Toast.LENGTH_LONG).show();
                            return;
                        }
                        ELog.d("====绑定教室==1111=");
                        mSelectClassPresenter.BindRooms(WiscApplication.prefs.getclassRoomId());
                        break;
                    case 0://取消
    ...
    public void showClossRooms(List<Classrooms> classroomses) {
        if(classroomses==null && classroomses.size()==0){
            Toast.makeText(SelectClassActivity.this, getResources().getString(R.string.no_find_roomlist), Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void showMessage(String msg) {
        Toast.makeText(SelectClassActivity.this, msg, Toast.LENGTH_LONG).show();
    ...
    public void showIsSuccess(Room mRoom) {
        if (mRoom != null && mRoom.name != null) {
            WiscApplication.prefs.setisBind(true);
